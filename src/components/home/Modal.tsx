...
export default function Modal({ searchPlace }: SearchPlaceProps) {
  ...
  return (
    <div
      ...
      bg-white rounded-t-3xl shadow-lg border border-border-main 
      ...
      <div className="w-full flex justify-center items-center py-3 cursor-grab">
        <div
          className={`w-12 h-1.5 ${
            mode === 1 ? "bg-border-main" : "bg-white"
          } rounded-full`}
        />
      </div>

      <div className="w-full flex flex-row justify-end px-5 text-gray-400 mb-1">
        <span
          onClick={() => changeMode(1)}
          className={`${mode === 1 ? "text-orange-main" : ""}`}
        >
          위치별
        </span>
        ...
        <span
          ...
          className={`${mode === 2 ? "text-orange-main" : ""}`}
        >
          매장별
        </span>
      </div>
